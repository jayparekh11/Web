...
const NotesPage = () => {
  ...
  const fetchNotes = async (keyword = '') => {
    try {
      const fetchedNotes = keyword.trim() ? await filterMeetingNotes({ keyword }) : await getAllMeetingNotes();
  ...
  return (
    <Box sx={{ p: theme.spacing(3) }}>
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
        ...
        />
        <Button variant="contained" color="primary" onClick={() => fetchNotes(searchKeyword)}>Search</Button>
      </Box>
      <Button
        variant="contained"
        color="secondary"
